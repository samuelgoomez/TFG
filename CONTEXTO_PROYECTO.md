...
**Limitación conocida, no arreglada**: en modo introducción se detectó, en 1 de 3 ejecuciones de prueba con respuestas deliberadamente vagas, que el coordinador puede delegar la tarea de validación *antes* de que el agente de adquisición haya guardado el informe en disco (`leer_informe_actual` devuelve "Todavía no existe ningún informe guardado"). El validador, al no encontrar nada, rechaza de inmediato con un "NO VALIDADO" que no refleja una evaluación real del contenido. En la ejecución donde pasó esto, el sistema no se recuperó y terminó rindiéndose por completo sin generar introducción; relanzando la misma prueba funcionó a la primera. No se ha arreglado porque el pipeline usa `Process.hierarchical`: el orden en que el coordinador delega en cada agente lo decide el LLM manager en cada ejecución, no una dependencia fija entre tareas — ya existe un `context=` en `tarea_validacion_intro` pensado para mitigar justo esto (ver commit `14b0d4d`) y aun así ha vuelto a pasar. Arreglarlo de raíz implicaría tocar la orquestación jerárquica, un cambio más arriesgado que el beneficio de corregir un fallo intermitente que se autorecupera al reintentar.

**Limitación detectada y arreglada**: probando el Escenario B (abstract, respuestas vagas sin cifras) se vio que el Agente de Validación de Completitud a veces da por bueno un "Resultados" sin ningún dato numérico, afirmando literalmente que sí cumple el criterio aunque no sea cierto — el mismo tipo de fallo que motivó el `_titulos_se_parecen` de `bib_writer.py`: no basta con pedirle al LLM que aplique una regla, hay que comprobarla por código. Se añadió un `guardrail` a `tarea_validacion` (`_verificar_resultados_con_dato_numerico` en `crew.py`) que rechaza por regex si el bloque de Resultados no tiene ninguna cifra, forzando un reintento automático de CrewAI. Al probarlo, el primer intento reveló un problema peor: presionado por el guardrail y sin ninguna cifra real disponible (el autor de prueba insistía en que no tenía datos), el agente se inventó un estudio completamente distinto con datos falsos con tal de pasar la validación. Se corrigió añadiendo una válvula de escape: si el autor insiste en que no tiene cifra, el informe debe usar el marcador `"[SIN DATO NUMÉRICO]"` en vez de inventar uno (instrucción en `tasks.yaml`, aceptado también por el guardrail). Solo afecta al modo interactivo del abstract; el modo PDF no lo necesita porque ya acepta "No especificado en el documento" y un reintento no podría conseguir un dato que el documento no tiene.

**Limitación detectada y arreglada (más grave que la anterior)**: probando el Escenario A (abstract, respuestas completas y correctas) se descubrió que, incluso con toda la información real disponible, el resultado final podía no tener nada que ver con la entrevista: en dos ejecuciones seguidas el abstract salió sobre "paneles solares" y sobre "sistemas HVAC", temas que el autor de prueba nunca mencionó. La causa: `tarea_estructuracion`, `tarea_redaccion`, `tarea_revision` y `tarea_control_calidad` no tenían agente asignado, así que el coordinador las ejecutaba delegando por su cuenta con `delegate_work_to_coworker`, y el mensaje de delegación que él mismo redactaba a veces no incluía el contenido real del informe validado. El especialista, sin datos reales a mano, se inventaba un estudio entero en vez de fallar. Se arregló asignando el agente directamente en `tasks.yaml` (`agent: agente_de_estructuracion_de_contenido`, `agente_redactor`, `agente_de_revision_de_estilo`, `agente_de_control_de_calidad`) y encadenando el `context=` real en `crew.py`, exactamente el patrón que introducción ya usaba desde el principio en sus bloques (Territorio, Hueco...) — por eso introducción nunca mostró este fallo pese a compartir la misma arquitectura de delegación jerárquica. Verificado relanzando el Escenario A varias veces tras el arreglo: el contenido se mantuvo siempre fiel al tema real. Confirmado también que no rompe el modo PDF (probado con *Attention Is All You Need* en ambos pipelines tras el cambio).

Las respuestas usadas para forzar y comprobar el bucle de validación (éxito y rechazo, tanto para abstract como para introducción) están en `escenarios_de_prueba.md`, reducido a los 4 casos imprescindibles: A (abstract, valida a la primera), B (abstract, fuerza rechazo), C (introducción, valida a la primera) y D (introducción, fuerza rechazo y repregunta). Los resultados quedaron guardados como ejemplo en `abstracts/generados/escenario_a_gradcam_generado.md`, `escenario_b_radiografias_generado.md`, `introducciones/generadas/escenario_c_codebert_generada.md` y `escenario_d_chatbot_generada.md` (y sus `.tex` correspondientes; excluidos de git igual que los `interactivo_*`, no son ejemplos del proyecto). Al ser modo interactivo sin carpeta de citas, ninguno de los dos escenarios de introducción genera `.bib`: las citas que menciona el escenario D ("Adiwardana et al. (2020)" para Meena, un dato real aunque el paper del escenario sea ficticio) quedan como texto narrativo sin `\cite{}`, que es el comportamiento esperado.
...
3. **`evidencias/` desactualizada.** El único PDF que hay ahora (`evidencias/attention_is_all_you_need.pdf`) es de una ejecución antigua: está en español, con el abstract vacío, título/autores sin rellenar y citas numeradas a mano en vez de `\cite{}` — no coincide con el Caso 1 (inglés) que describe la memoria en la Sección 6.1.1. Hay que compilar `introducciones/latex/attention_is_all_you_need.tex` (con bibliografía real) y sustituir el PDF, e idealmente añadir la evidencia compilada de los otros 3 casos (SimulateIoT, Párkinson, BERT) para respaldar el RNF-9.1 al completo.
